...
import { exportClientesFile } from "../../../../lib/services/clienteService";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { buffer, contentType, filename } = await exportClientesFile({ format, scope, ids });
    ...
    console.error("Error GET /cliente/export:", error);
    return jsonCors({ error: "Error al exportar clientes" }, { status: 500 });
  }
}
