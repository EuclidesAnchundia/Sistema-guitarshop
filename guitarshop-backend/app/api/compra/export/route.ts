...
import { exportComprasFile } from "../../../../lib/services/compraService";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { buffer, contentType, filename } = await exportComprasFile({ format, scope, ids });
    ...
    console.error("Error GET /compra/export:", error);
    return applyCorsHeaders(NextResponse.json({ error: "Error al exportar compras" }, { status: 500 }));
  }
}
