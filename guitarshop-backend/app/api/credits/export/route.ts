...
import { exportCreditsFile } from "../../../../lib/services/creditoService";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { buffer, contentType, filename } = await exportCreditsFile({ format, scope, ids });
    ...
    console.error("Error GET /credits/export:", error);
    return applyCorsHeaders(NextResponse.json({ error: "Error al exportar créditos" }, { status: 500 }));
  }
}
