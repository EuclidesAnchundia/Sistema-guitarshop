...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { buffer, contentType, filename } = await exportProveedoresFile({ format, scope, ids });
    ...
    console.error("Error GET /proveedor/export:", error);
    return applyCorsHeaders(NextResponse.json({ error: "Error al exportar proveedores" }, { status: 500 }));
  }
}
